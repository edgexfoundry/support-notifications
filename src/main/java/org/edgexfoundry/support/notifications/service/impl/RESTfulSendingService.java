...
public class RESTfulSendingService extends AbstractSendingService {
	...
	TransmissionRecord sendToReceiver(Notification notification, Channel channel) {
		...
		headers.setContentType(notification.getContentType() == null ?
				MediaType.TEXT_PLAIN : MediaType.parseMediaType(notification.getContentType()));
		HttpEntity<String> request = new HttpEntity<String>(notification.getContent(), headers);

		TransmissionRecord record = new TransmissionRecord();

		logger.debug("sending notification to " + restfulChannel.getUrl() + " by HTTP " + restfulChannel.getHttpMethod()
				+ " method");
		...
		try {
			...
			logger.debug(
					"got response status code: " + response.getStatusCode() + " with content: " + response.getBody());
